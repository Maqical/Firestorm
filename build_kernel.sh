...
cp -R ./maqical ${KERNELDIR}/output/$TARGET/
cp -R ./supersu ${KERNELDIR}/output/$TARGET/
cp -R ./busybox ${KERNELDIR}/output/$TARGET/
cp ./$TARGET/boot.img ${KERNELDIR}/output/$TARGET/maqical
...
tar -H ustar -c ${KERNELDIR}/output/$TARGET/maqical/boot.img > SM-$TARGET-$AUDIO-kernel-${GETVER}-`date +[%d-%m-%y]`.tar
